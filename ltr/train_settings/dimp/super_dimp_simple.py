...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq, TOT
...
def run(settings):
    ...
    SOT_train = TOT(settings.env.latot_dir)
    SOT_val = TOT(settings.env.latot_test_dir)
    ...
    dataset_train = sampler.DiMPSampler([trackingnet_train,lasot_train,got10k_train,SOT_train], [1,1,1,3],
    ...
    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size, num_workers=settings.num_workers,
                           shuffle=False, drop_last=True, epoch_interval=55, stack_dim=1)
